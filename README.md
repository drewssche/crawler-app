...
- Основная вкладка показывает компактную сводку по сайтам, контекстам, расписанию, отслеживаемым блокам, уведомлениям и участникам. Элементы сводки открывают соответствующую секцию, а выбранные вкладка и раздел сохраняются в URL.
- Глобальная готовность worker и общая очередь доставки не смешиваются с данными проекта; в проекте остаются только его собственные состояния.
...
В Compare можно выбрать визуальный фрагмент страницы — например цену, кнопку, баннер или контактный блок — и добавить его в отслеживание. После следующих успешных прогонов crawler автоматически проверяет, остался ли блок на месте.
...
Для отслеживаемых блоков доступны:
...
- delivery outbox;
- retry/backoff;
- глобальная диагностика доставки для admin/root-admin вынесена из страницы проекта;
