...
- [ ] Friendly UX/UI refinement wave (`P1`, minimal UI pass перед расширением новых фич):
  ...
  - Этап 6 follow-up — Event center task queue: позже добавить в правый sidebar третью область `Очередь задач` с тем же поведением режимов; если открыты все области одновременно, layout должен делить высоту примерно 33/33/33, сохраняя отдельные full-height режимы для каждой области.
