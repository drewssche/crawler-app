...
- [ ] Friendly UX/UI refinement wave (`P1`, minimal UI pass перед расширением новых фич):
  ...
  - Этап 4 follow-up — Project settings internals: дочистить внутренние формы `ProjectSitesSettings`/`ProjectMembersSettings`: заменить оставшиеся native select на chips/combobox там, где это не ломает сложные формы, и вынести справку в будущий `?`.
