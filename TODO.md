...
- [ ] Friendly UX/UI refinement wave (`P1`, minimal UI pass перед расширением новых фич):
  ...
  - Этап 2 follow-up — Structure workspace: для multi-context структуры решить модель отображения нескольких runs одновременно отдельными секциями, потому что сейчас UI строит дерево по одному выбранному run.
