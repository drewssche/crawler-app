...
- [ ] Friendly UX/UI refinement wave (`P1`, minimal UI pass перед расширением новых фич):
  ...
  - Этап 1 — Project main minimal UI: оставить вверху название + статус + структурированные действия; убрать пояснительные тексты, `Рабочую сводку`, `Показатели последнего прогона`, отдельный блок контекста просмотра; карточки сайтов сократить до адреса, контекста-чипа, статуса, количества прогонов, страниц/изменений по правилам истории.
  - Этап 2 — Structure workspace: контекст просмотра и домены перевести в checkbox/chip filters внутри структуры; для малого числа доменов — chips, для большого — группировка по доменам; расширить поиск по title/description/h1/final URL после проверки доступности этих полей в данных.
