...
- [ ] **P1 EPIC: Site monitoring + scoped crawl + compare workspace** (`HIGH`, staged).
  ...
  **Crawl personas / user contexts**
  ...
  - staged implementation:
    ...
    3. browser-based login scenario для CSRF/dynamic forms — foundation готов: Playwright runtime умеет применять persona browser state (`cookies/localStorage/sessionStorage/headers`) в runtime consent audit; login capture API contract готов (`start/status/complete/cancel`, без хранения secrets в capture-row); UI wizard MVP готов: `Открыть сайт и войти` + ручная вставка Playwright storageState; следующий шаг — автоматический захват storageState из управляемого браузера;
