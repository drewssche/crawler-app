...
- [ ] **Project overview + information architecture cleanup** (`P1`, согласовано 2026-07-21).
  ...
  **Stage 1 — Понятная модель отслеживания**
  - [ ] переименовать пользовательский термин `Цели мониторинга` в `Отслеживаемые блоки`: это сохранённые фрагменты страницы (кнопка, цена, баннер, текстовый/контактный блок), которые crawler ищет после следующих прогонов и помечает как `На месте / Изменился / Не найден / Ждёт проверки`;
  - [ ] в техническом/API-контракте `monitoring target` пока можно сохранить, но не выводить этот термин пользователю без расшифровки;
  - [ ] секцию `Отслеживаемые блоки` располагать выше `Структуры сайта`: длинное дерево не должно отодвигать управление отслеживанием вниз страницы;
  - [ ] zero-state показывать нейтрально (`Блоки не добавлены`), без warning-tone; при наличии блоков показывать количество и только реальные проблемы;
  ...
  **Stage 2 — Кликабельная сводка проекта**
  - [ ] под header добавить компактную responsive-панель status shortcuts: `Сайты`, `Контексты`, `Расписание`, `Отслеживаемые блоки`, `Уведомления`, `Участники`;
  - [ ] каждый shortcut ведёт в точную область по контексту: настройки сайтов/personas, расписание, участники, каналы либо раскрытие отслеживаемых блоков;
  - [ ] закрепить navigation state в URL (`tab`/`section` или nested route), чтобы deep-link, refresh и кнопка браузера `Назад` сохраняли выбранную область;
  - [ ] цвет использовать семантически: neutral — информация/необязательная пауза, success — настроено и работает, warning — требуется решение, danger — реальная ошибка; не делать все shortcuts цветными;
  - [ ] `Уведомления не настроены` считать предупреждением только когда у проекта есть активные отслеживаемые блоки/подписки, которым требуется внешний канал.

  **Stage 3 — Убрать системный шум из проекта**
  - [ ] удалить из project main глобальные diagnostics crawler/worker и notification outbox: они считают состояние всей системы/всех проектов и внутри конкретного проекта вводят в заблуждение;
  - [ ] глобальную готовность crawler, каналов Email/Telegram, retry/backoff и outbox перенести в admin/root-admin раздел `Состояние системы`/`Доставка уведомлений`; в проекте показывать только project-scoped состояние;
  - [ ] не показывать healthy/zero operational KPI постоянно: сворачивать их в одно состояние (`Очередь пуста`) либо полностью скрывать до появления проблемы;
  - [ ] убрать дубли KPI (`крупный 0` + chip `0/Есть`); оставлять одно число, а смысл состояния передавать label, тоном числа/иконкой/рамкой;
  - [ ] `Остановлены` переименовать в `Не доставлены`, `Ждут паузу` — в `Следующая попытка позже`, `Готовы к повтору` — в `Готовы к повторной отправке`; cumulative `Отправлены` не смешивать с текущей очередью без периода (`за 24 часа` или отдельная история).

  **Stage 4 — Целевая композиция основной вкладки**
  - [ ] порядок блоков: header + primary actions → status shortcuts → `Требует внимания` (только при проблемах) → карточки сайтов → отслеживаемые блоки → структура сайта;
  - [ ] live job/run state показывать рядом со структурой и выбранным сайтом, не дублировать очередь в нескольких местах;
  - [ ] карточки сайтов оставить рабочим переключателем контекста, но не повторять name/domain двумя строками, если они совпадают; показывать persona, scope, runs, результат последнего прогона и следующий запуск только при наличии;
  - [ ] историю оставить компактной: дата, результат, persona, страницы, изменения, длительность; run id, точные timestamps/current URL/failure details держать в раскрываемых деталях;
  - [ ] при отсутствии проблем не создавать отдельные пустые warning/info-блоки: основной экран должен оставаться коротким и ориентированным на действие.
