...
- [ ] Friendly UX/UI refinement wave (`P1`, перед расширением новых фич):
  ...
  - создание проекта: одинаковый URL в разных проектах разрешён; duplicate scope внутри одного проекта блокируется/объясняется; похожее название проекта получает мягкое предупреждение. Форма создания поддерживает `+ Добавить сайт`, показывает карточки сайтов и итог “будет создано N сайтов”.
  - проект: вкладки `Основная / История / Настройки` поднять над контентом и визуально оформить как главный переключатель страницы; на `Основной` оставить ежедневную работу, технические/админские детали свернуть; `Запустить все сайты` скрывать при одном сайте.
  - настройки проекта: сгруппировать accordion-секциями `Сайты`, `Участники`, `Расписание`, `Лимиты и хранение`, `Опасная зона`; расписание до backend-контракта остаётся честным manual-only состоянием.
  - Settings: `Лимиты ролей` и `Хранилище сканов` спрятать в раскрываемые секции, русифицировать роли/метрики, добавить понятные подсказки; редактирование лимитов позже делать отдельным root-admin backend-контрактом с audit log, admin пока read-only.
  - роли/чипы: везде использовать единый словарь `Наблюдатель / Редактор / Администратор / Root-admin`; чип `Вы` показывать сразу после email/имени пользователя в Users, RootAdmins, project members и drawer.
  - участники проекта: поле добавления участника заменить на searchable combobox по существующим пользователям с состояниями `уже участник / не найден / заблокирован / ожидает подтверждения`.
  - глобальная очередь задач: сделать почти везде доступной как collapsible side panel/rail рядом с существующими сайдбарами, кроме full-screen Compare workspace. Viewer видит read-only задачи доступных проектов, editor может отменять/открывать свои проектные задачи, admin/root-admin видит расширенную operational-диагностику. Пустое состояние тоже показывать компактно.
  - Compare flow: шаг выбора страниц отделить от workspace. Auto-match показывает spinner/рекомендацию и цветовое выделение, затем кнопка `Сравнить` открывает focus workspace без сайдбаров; контекст страниц показывать на шаге выбора или по кнопке `Информация`, не постоянными панелями. В режиме выбора блоков hover явно подсвечивает выбираемый блок.
