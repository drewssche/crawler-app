...
- [ ] Friendly UX/UI refinement wave (`P1`, minimal UI pass перед расширением новых фич):
  ...
  - 2026-06-30 stage 2 partial: контекст просмотра и домены перенесены в `Структуру сайта` chips-контролами; домены поддерживают мультивыбор, поиск расширен по доступным полям (`URL`, `final URL`, домен, HTTP/status/error). SEO-поля `title/description/h1` в `/runs/{id}/pages` пока не отдаются — для них нужен backend/API контракт.
  - Этап 2 follow-up — Structure workspace: добавить backend/API поля `title/description/h1` в lightweight список страниц или отдельный search endpoint; для multi-context структуры решить модель отображения нескольких runs одновременно отдельными секциями, потому что сейчас UI строит дерево по одному выбранному run.
  - Этап 3 — Project history minimal cards: убрать дублирующие info-блоки, карточки прогонов перевести на чипы/иконки (`статус`, `контекст`, `runtime`, `страницы`, `изменения`, `ошибки`, `длительность`), детали только по раскрытию/клику.
  - Этап 4 — Project settings navigation: после визуальной проверки заменить accordion на карточки-разделы/внутренние страницы по аналогии с глобальными Settings; убрать справочные тексты внутрь `?`, заменить оставшиеся select на chips/combobox.
  - Этап 5 — Compare wizard: выбор левой страницы → выбор правой страницы → кнопка `Сравнить`; автоподбор как явный toggle, spinner внутри контролов, auto-picked состояния подсвечивать, режимы `Визуально/Код/Структура` показывать только после выбора обеих страниц.
  - Этап 6 — Sidebar UX: левый sidebar получает стабильный collapse/expand; правый sidebar — фиксированную toggle-кнопку и режимы `Уведомления / Лента`, где выбранный блок занимает всю высоту.
  - Этап 7 — Interaction polish: единый hover/active стиль для кликабельных cards/disclosure/section rows; keyboard polish combobox (`↑/↓/Enter`, ARIA active descendant) после визуальной проверки.
