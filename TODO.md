...
- Следующий рекомендуемый пункт: **post-review UX cleanup**: ручной аудит выявил не фичевые пробелы, а перегруженность интерфейса, повторяющиеся подсказки и несколько раздражающих interaction bugs. Сначала чистить шум и консистентность, затем возвращаться к Browser-smoke по явному запросу пользователя.
...
- [ ] **Project overview + information architecture cleanup** (`P1`, согласовано 2026-07-21).
  Цель: основная вкладка проекта отвечает на четыре вопроса — что выбрано, как прошёл последний прогон, что требует внимания и куда перейти для запуска/исследования/настройки. Не смешивать состояние проекта, выбранного сайта и глобальную диагностику системы.

  **Stage 1 — Понятная модель отслеживания**
  - [x] переименовать пользовательский термин `Цели мониторинга` в `Отслеживаемые блоки`: это сохранённые фрагменты страницы (кнопка, цена, баннер, текстовый/контактный блок), которые crawler ищет после следующих прогонов и помечает как `На месте / Изменился / Не найден / Ждёт проверки`;
  - [x] в техническом/API-контракте `monitoring target` пока можно сохранить, но не выводить этот термин пользователю без расшифровки;
  - [x] секцию `Отслеживаемые блоки` располагать выше `Структуры сайта`: длинное дерево не должно отодвигать управление отслеживанием вниз страницы;
  - [x] zero-state показывать нейтрально (`Блоки не добавлены`), без warning-tone; при наличии блоков показывать количество и только реальные проблемы;
  - [ ] создание блока должно быть доступно не только из Compare: предусмотреть понятный переход из empty-state и сохранение выделенного элемента из анализа одной страницы.

  **Stage 2 — Кликабельная сводка проекта**
  - [x] под header добавить компактную responsive-панель status shortcuts: `Сайты`, `Контексты`, `Расписание`, `Отслеживаемые блоки`, `Уведомления`, `Участники`;
  - [x] каждый shortcut ведёт в точную область по контексту: настройки сайтов/personas, расписание, участники, каналы либо раскрытие отслеживаемых блоков;
  - [x] закрепить navigation state в URL (`tab`/`section` или nested route), чтобы deep-link, refresh и кнопка браузера `Назад` сохраняли выбранную область;
  - [x] цвет использовать семантически: neutral — информация/необязательная пауза, success — настроено и работает, warning — требуется решение, danger — реальная ошибка; не делать все shortcuts цветными;
  - [x] отсутствие внешнего Email/Telegram не считать ошибкой: без подписок показывать нейтральное `В приложении`, а активные project-scoped каналы считать по отслеживаемым блокам.

  **Stage 3 — Убрать системный шум из проекта**
  - [x] удалить из project main глобальные diagnostics crawler/worker и notification outbox: они считают состояние всей системы/всех проектов и внутри конкретного проекта вводят в заблуждение;
  - [x] глобальную готовность crawler, каналов Email/Telegram и текущую доставку перенести в admin/root-admin `Мониторинг → Состояние системы`; в проекте показывать только project-scoped состояние;
  - [x] не показывать healthy/zero operational KPI постоянно: сворачивать их в одно состояние (`Очередь пуста`) либо полностью скрывать до появления проблемы;
  - [x] убрать дубли KPI (`крупный 0` + chip `0/Есть`); оставлять одно число, а смысл состояния передавать label, тоном числа/иконкой/рамкой;
  - [x] `Остановлены` переименовать в `Не доставлены`, `Ждут паузу` — в `Следующая попытка позже`, `Готовы к повтору` — в `Готовы к повторной отправке`; cumulative `Отправлены` не смешивать с текущей очередью без периода.

  **Stage 4 — Целевая композиция основной вкладки**
  - [x] порядок блоков: header + primary actions → status shortcuts → `Требует внимания` (только при проблемах) → карточки сайтов → отслеживаемые блоки → структура сайта;
  - [x] live job/run state показывать рядом со структурой и выбранным сайтом, не дублировать глобальную очередь в нескольких местах;
  - [x] карточки сайтов оставить рабочим переключателем контекста, но не повторять name/domain двумя строками, если они совпадают; показывать persona, scope, runs и результат последнего прогона;
  - [x] историю оставить компактной: дата, результат, persona, страницы, изменения, длительность; run id, точные timestamps/current URL/failure details держать в раскрываемых деталях;
  - [x] при отсутствии проблем не создавать отдельные пустые warning/info-блоки: основной экран должен оставаться коротким и ориентированным на действие.

  Evidence 2026-07-21: frontend build и targeted lint passed; project UI contract `34 passed`; backend source compile passed. Manual Browser-smoke не запускался по договорённости.

- [ ] **Beta deploy access without SMTP** (`P1`, 2026-07-02).
  Контекст: на первом beta-деплое SMTP может быть не настроен, поэтому email-code/invite по письму не покрывает доступ внешних тестеров. Нужен временный, контролируемый fallback без потери будущего email-code flow.
  - [ ] root-admin остаётся закреплён через env (`ADMIN_EMAILS`, `EMERGENCY_ROOT_ADMIN_EMAIL`, `ADMIN_PASSWORD`);
  - [ ] добавить вход пользователя по временному паролю, включаемый отдельным env-флагом только для beta;
...
- [ ] **Post-review UX cleanup wave** (`P1`, screenshots 2026-07-02).
  ...
  **Stage 1 — Toast/Event Center**
  - [x] сделать toast менее прозрачным и визуально плотным;
  - [x] привести primary/secondary actions toast к одинаковой ширине/ритму;
  - [x] заменить линейный таймер закрытия на progress с завершающим spinner-marker;
  - [x] не показывать повторный toast для уже полученного Event Center item при переходах между проектом, анализом страницы и назад;
  - [x] если Event Center сам показывает старое событие — это нормально, но всплывающий toast должен появляться только для нового события.

  **Stage 2 — Project header + operational cleanup**
  - [x] заменить служебные Vite title/favicon на бренд `Crawler App` и фирменный знак crawler;
  - [x] на основной странице проекта, в сравнении и анализе страницы показывать browser title `Название проекта · Crawler App`, возвращая базовый title после выхода из проекта;
  - [x] предусмотреть длинное название проекта без поломки header;
  - [x] перенести status-chip выше/рядом компактно, без давления на title;
  - [x] выровнять `Запуск`, `Запустить выбранный сайт`, `Сравнить страницы` в понятный action cluster;
  - [x] убрать дублирующий operational/status блок, если его данные уже видны в title/chips/right sidebar;
  - [x] упростить notification delivery diagnostics: оставить статусы и счётчики, длинные пояснения вынести в будущую контекстную базу знаний.

  **Stage 3 — Monitoring targets + settings noise**
  - [x] `Цели мониторинга` сделать компактной/сворачиваемой секцией с количеством целей;
  - [x] empty state без длинной инструкции: подробности позже через `?`/knowledge help;
  - [x] убрать flicker diagnostics при переключении вкладок проекта;
  - [x] привести русский/английский нейминг KPI и chips к единому стилю.

  **Stage 4 — Structure performance + semantics**
  - [x] при переключении `Все/Гость` не перезагружать структуру, если доступен только один контекст;
  - [x] проверить соответствие иконок дерева реальной легенде;
  - [x] оставить в Structure легенду и дерево, а технические детали среза свернуть/сократить;
  - [x] доработать поиск по URL/title/description/H1/final URL/status/error с заметным жёлтым highlight.

  **Stage 5 — Drawers**
  - [x] контекст раздела сделать кратким: состав, состояние вложенных страниц, действия;
  - [x] контекст страницы выровнять в header: title/URL слева, actions справа одним аккуратным блоком;
  - [x] длинные meta description/technical details показывать с ограничением и раскрытием, чтобы drawer не превращался в стену текста.

  **Stage 6 — Compare workspace polish**
  - [x] заменить самые шумные controls на custom/chip-pickers: page picker уже searchable, контекст доступа переведён на chips; site/run остаются styled select до отдельного searchable combobox, чтобы не ухудшить длинные списки;
  - [x] явно показать пошаговость выбора, spinner auto-match и опцию отключить автоподбор;
  - [x] после выбора двух страниц открывать dedicated compare workspace по кнопке `Сравнить`, без лишних sidebars/context panels.

  **Stage 7 — Sidebars**
  - [x] добавить левому sidebar collapse/expand;
  - [x] стабилизировать кнопку правого sidebar, чтобы она не прыгала при collapse;
  - [x] правый sidebar поддерживает режимы `оба блока 50/50`, `только уведомления`, `только лента`, позднее `очередь задач`; если три области открыты — по 33%.

- [ ] **Release stabilization / manual Browser-smoke gate**.
  Technical checks по MVP закрыты. Ручной Browser-smoke ключевых сценариев выполнять только по явному запросу пользователя.
