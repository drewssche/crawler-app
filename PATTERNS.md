...
- Для `UsersPage` и `UserDetailsDrawer` используем единые reusable-блоки:
  ...
  Краткую строку сессии в списках (`IP • устройство • активность`) рендерим через общий `UserListSessionMeta` и используем одинаково в `UsersPage` и `RootAdminsPage`.
  Для консистентной высоты карточек списков `UserListSessionMeta` рендерится всегда: при отсутствии данных показываем прочерки (`IP: - • устройство: - • активность: -`).
