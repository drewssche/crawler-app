# REUSE_INDEX — full snapshot 2026-06-22
...
- `frontend/src/components/ui/HighlightedText.tsx`
  Shared подсветка match-фрагментов поискового запроса (case-insensitive, `ё/е`, безопасный split без regex-инъекций).
  Подключение: `ProfileDashboardPage -> Структура`, `WorkspaceHomePage`, `SidebarLeft`, `ProjectDomainPills`.

- `frontend/src/utils/projectSearch.ts`
  Единый project-search matcher/ranker для `WorkspaceHomePage` и `SidebarLeft`:
  `NFKC + lowercase + ё/е + URL/domain cleanup + RU/EN keyboard recovery`;
  ranking `exact > prefix/segment-prefix > substring > layout-match` со стабильным исходным порядком при равенстве.
  Возвращает effective highlight query и matched field/value, чтобы UI показывал объяснимое совпадение без локальной поисковой логики.
