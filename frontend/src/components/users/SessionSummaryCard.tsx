...
export default function SessionSummaryCard({
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      ...
      <MetaText size={13} opacity={0.88}>
        {"\u0430\u043a\u0442\u0438\u0432\u043d\u043e\u0441\u0442\u044c"}: {lastActivityAt ? formatApiDateTime(lastActivityAt) : "-"}
      </MetaText>
      <MetaText size={13} opacity={0.88}>IP: {lastIp || "-"}</MetaText>
      <MetaText size={13} opacity={0.88} title={lastUserAgent || "-"}>
        {"\u0443\u0441\u0442\u0440\u043e\u0439\u0441\u0442\u0432\u043e"}: {shortUserAgent(lastUserAgent)}
      </MetaText>

      <MetaText size={13} opacity={0.82} style={{ marginTop: 2 }}>
      ...
      <div style={{ marginTop: 4, borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 8 }}>
        ...
        {!latestLogin && <MetaText opacity={0.72}>{"\u0434\u0430\u043d\u043d\u044b\u0445 \u043d\u0435\u0442"}</MetaText>}
        {latestLogin && (
          <>
            <MetaText opacity={0.84}>{latestLogin.result}{UI_BULLET}{latestLogin.source}</MetaText>
            <MetaText opacity={0.84}>{"\u043a\u043e\u0433\u0434\u0430"}: {latestLogin.created_at ? formatApiDateTime(latestLogin.created_at) : "-"}</MetaText>
            <MetaText opacity={0.84}>IP: {latestLogin.ip || "-"}</MetaText>
            <MetaText opacity={0.84} title={latestLogin.user_agent || "-"}>
